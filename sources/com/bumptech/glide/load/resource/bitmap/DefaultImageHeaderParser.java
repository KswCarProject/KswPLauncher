...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private ImageHeaderParser.ImageType getType(Reader reader) throws IOException {
        ...
        if (firstFourBytes == PNG_HEADER) {
            ...
            reader.skip(4L);
            int thirdFourBytes = ((reader.getUInt16() << 16) & SupportMenu.CATEGORY_MASK) | (reader.getUInt16() & 65535);
    ...
    private static final class StreamReader implements Reader {
        ...
        /* renamed from: is */
        private final InputStream f87is;
        ...
        public int getUInt16() throws IOException {
            return ((this.f87is.read() << 8) & MotionEventCompat.ACTION_POINTER_INDEX_MASK) | (this.f87is.read() & 255);
        }

        @Override // com.bumptech.glide.load.resource.bitmap.DefaultImageHeaderParser.Reader
        public short getUInt8() throws IOException {
            return (short) (this.f87is.read() & 255);
        }
        ...
        public long skip(long total) throws IOException {
            ...
            long toSkip = total;
            while (toSkip > 0) {
                long skipped = this.f87is.skip(toSkip);
                if (skipped > 0) {
                    toSkip -= skipped;
                } else {
                    int testEofByte = this.f87is.read();
                    if (testEofByte == -1) {
                        break;
                    }
                    toSkip--;
        ...
        public int read(byte[] buffer, int byteCount) throws IOException {
            ...
            while (toRead > 0) {
                int read = this.f87is.read(buffer, byteCount - toRead, toRead);
                if (read == -1) {
                    break;
                }
                toRead -= read;
