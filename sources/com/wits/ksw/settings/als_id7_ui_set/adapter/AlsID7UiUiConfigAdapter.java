...
public class AlsID7UiUiConfigAdapter extends RecyclerView.Adapter<MyViewHolder> {
    ...
    public AlsID7UiUiConfigAdapter(Context context, List<FunctionBean> data) {
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
        MyViewHolder holder = new MyViewHolder(LayoutInflater.from(this.context).inflate(C0899R.C0902layout.als_id7_ui_list_tv_layout, viewGroup, false));
        return holder;
    }
    ...
    public void onBindViewHolder(final MyViewHolder holder, final int position) {
        ...
        if (this.data.get(position).isIscheck()) {
            holder.tv_mgs.setTextColor(SkinCompatResources.getColor(this.context, C0899R.color.als_id7_ui_text_color));
        } else {
            holder.tv_mgs.setTextColor(-1);
        }
        if (this.rbtCheckListener != null) {
            holder.relat_listTv.setOnClickListener(new View.OnClickListener() { // from class: com.wits.ksw.settings.als_id7_ui_set.adapter.AlsID7UiUiConfigAdapter.1
        ...
        holder.relat_listTv.setOnKeyListener(new View.OnKeyListener() { // from class: com.wits.ksw.settings.als_id7_ui_set.adapter.AlsID7UiUiConfigAdapter.2
            @Override // android.view.View.OnKeyListener
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                if (event.getKeyCode() != 20) {
                    return event.getKeyCode() == 19 && position == 0;
                } else if (AlsID7UiUiConfigAdapter.this.data.size() - 1 == position) {
