...
final class RunSuspend implements Continuation<Unit> {
    ...
    public final Result<Unit> m1280getResultxLWZpok() {
        return this.result;
    }
    ...
    public void resumeWith(Object result) {
        synchronized (this) {
            this.result = Result.m92boximpl(result);
            notifyAll();
    ...
    public final void await() {
        synchronized (this) {
            while (true) {
                Result result = m1280getResultxLWZpok();
                if (result == null) {
                    wait();
                } else {
                    ResultKt.throwOnFailure(result.m102unboximpl());
                }
            }
        }
    }
